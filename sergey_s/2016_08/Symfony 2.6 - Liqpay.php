...
namespace FeatureBundle\PaymentBundle\Service;

use Symfony\Component\Security\Acl\Exception\Exception;
use Symfony\Component\DependencyInjection\ContainerInterface as Container;
use FeatureBundle\PaymentBundle\Entity\PaymentLog as PaymentLog;
...
class Liqpay
{
    ...
    public function storePaymentLog($response)
    {
        $paymentLogSource = json_decode($response);
        unset($paymentLogSource->paytype);
        $log  = new PaymentLog();

        foreach ($paymentLogSource as $field => $value) {
            $method = "set".Inflector::camelize($field);
            $log->$method($value);
        }

        $eManager = $this->container->get('doctrine.orm.default_entity_manager');
